...
async function createLinkedMentionFiles(
  page,
  { count = 12, target = "projects/collabmd" } = {},
) {
...
test("keeps preview and outline aligned when scrolling list-heavy editor content", async ({
  page,
}) => {
  ...
  await expect(page.locator("#previewContent")).toContainText(
    "Second section item 80.",
  );

  await page.locator("#outlineToggle").click();
  await expect(page.locator("#outlinePanel")).toBeVisible();

  const targetEditorLine = page
    .locator(".cm-line", { hasText: "Second section item 52 sync target." })
    .first();
  ...
  await expect
    .poll(async () => {
      const activeItem = page
        .locator("#outlineNav .outline-item.active")
        .first();
      return activeItem.textContent();
    })
    .toContain("Second section");

  const targetPreviewOffset = await page
    .locator("#previewContent li", {
      hasText: "Second section item 52 sync target.",
    })
    .evaluate((item) => {
...
test("scrolls the editor to the selected heading when navigating from the outline", async ({
  page,
}) => {
  await openFile(page, "README.md");
  ...
  await expect(page.locator("#outlinePanel")).toBeVisible();
  await page
    .locator("#outlineNav .outline-item", { hasText: "Second section" })
    .click();

  const editorHeadingOffset = await page
    .locator(".cm-line", { hasText: "## Second section" })
    .first()
    .evaluate((line) => {
  ...
  const previewHeadingOffset = await page
    .locator("#previewContent h2", { hasText: "Second section" })
    .evaluate((heading) => {
      const container = document.getElementById("previewContainer");
      const containerRect = container.getBoundingClientRect();
  ...
  await expect(
    page.locator("#outlineNav .outline-item.active").first(),
  ).toHaveText("Second section");
});

test("keeps the preview pinned to the top when a file first opens at the top of the editor", async ({
  page,
}) => {
  await openFile(page, "showcase.md");
  await expect(
    page.locator("#previewContent h1", { hasText: "CollabMD Workspace Tour" }),
  ).toBeVisible();
  ...
  const previewHeadingOffset = await page
    .locator("#previewContent h1", { hasText: "CollabMD Workspace Tour" })
    .evaluate((heading) => {
      const container = document.getElementById("previewContainer");
      const containerRect = container.getBoundingClientRect();
      const headingRect = heading.getBoundingClientRect();
...
test("keeps the linked mentions dock reachable while preview scrolls and expands it without moving the document", async ({
  page,
}) => {
  ...
  await expect
    .poll(async () =>
      Number.parseInt(
        (await dock.locator(".backlinks-count").textContent()) || "0",
        10,
      ),
    )
    .toBeGreaterThan(10);

  const measureDockAlignment = async () =>
    page.evaluate(() => {
      const dockElement = document.querySelector(
        "#backlinksPanel .backlinks-panel-dock",
      );
  ...
  expect(
    Math.abs(positions[0].bottomOffset - positions[1].bottomOffset),
  ).toBeLessThanOrEqual(2);
  expect(
    Math.abs(positions[1].bottomOffset - positions[2].bottomOffset),
  ).toBeLessThanOrEqual(2);
  ...
  const scrollTopBeforeExpand = await page
    .locator("#previewContainer")
    .evaluate((element) => element.scrollTop);
  await dockHeader.click();
  await expect(dock).toHaveClass(/expanded/);

  const scrollTopAfterExpand = await page
    .locator("#previewContainer")
    .evaluate((element) => element.scrollTop);
  expect(
  ...
  const bodyMetrics = await dock
    .locator(".backlinks-body")
    .evaluate((element) => ({
...
test("keeps the clicked parent heading active in the outline after navigation", async ({
  page,
}) => {
  await openFile(page, "showcase.md");
  await page.locator("#outlineToggle").click();
  ...
  await page
    .locator("#outlineNav .outline-item", { hasText: "Embedded Diagram Files" })
    .click();
...
test("keeps the outline open on desktop after selecting a section", async ({
  page,
}) => {
  await openFile(page, "README.md");
...
test("keeps the preview container width stable when the outline opens in preview mode", async ({
  page,
}) => {
  await openFile(page, "README.md");
  await replaceEditorContent(page, OUTLINE_TEST_DOCUMENT);
...
test("keeps the preview container width stable when the outline opens in split mode", async ({
  page,
}) => {
  await openFile(page, "README.md");
  await replaceEditorContent(page, OUTLINE_TEST_DOCUMENT);
...
test("keeps the editor interactive before heavy preview reaches ready", async ({
  page,
}) => {
  test.slow();
  ...
  const phase = await page
    ...
    .getAttribute("data-render-phase");
...
test("progressively hydrates heavy preview instead of rendering all embeds at once", async ({
  page,
}) => {
  test.slow();
  ...
  expect(
    counts.excalidrawPlaceholders + counts.excalidrawIframes,
  ).toBeGreaterThan(0);
  expect(counts.mermaidSvgs).toBeLessThan(1431);
  expect(counts.excalidrawIframes).toBeLessThan(159);
});

test("hydrates more mermaid and excalidraw content as the heavy preview scrolls", async ({
  page,
}) => {
  test.slow();
  ...
  const mermaidIncreased =
    middle.mermaidSvgs > before.mermaidSvgs ||
    after.mermaidSvgs > middle.mermaidSvgs;
  const excalidrawIncreased =
    middle.excalidrawIframes > before.excalidrawIframes ||
    after.excalidrawIframes > middle.excalidrawIframes;
  const layoutExpanded =
    middle.scrollHeight > before.scrollHeight ||
    after.scrollHeight > middle.scrollHeight;

  expect(
    mermaidIncreased || excalidrawIncreased || layoutExpanded,
  ).toBeTruthy();
});

test("defers heavy preview hydration while the editor is actively scrolling", async ({
  page,
}) => {
  test.slow();
...
test("keeps editor, preview, and outline aligned in heavy documents after lazy hydration changes layout", async ({
  page,
}) => {
  test.slow();
  ...
  const outlineItems = page.locator(
    "#outlineNav .outline-item[data-source-line]",
  );
  await expect
    .poll(async () => outlineItems.count(), { timeout: 60000 })
    .toBeGreaterThan(0);

  const outlineItemCount = await outlineItems.count();
  const targetIndex =
    outlineItemCount >= 3 ? outlineItemCount - 3 : outlineItemCount - 1;
  const targetOutlineItem = outlineItems.nth(targetIndex);
  await expect(targetOutlineItem).toBeVisible({ timeout: 60000 });

  const targetLine = Number.parseInt(
    (await targetOutlineItem.getAttribute("data-source-line")) || "",
    10,
  );
  ...
  await expect
    .poll(async () => getPreviewHeadingOffset(page, targetLine), {
      timeout: 60000,
    })
    .toBeLessThan(260);
});
