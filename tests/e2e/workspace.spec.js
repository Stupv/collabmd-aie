...
test("prompts first-time visitors for a display name", async ({ browser }) => {
  ...
  await expect(page.locator("#displayNameTitle")).toHaveText(
    "Choose your display name",
  );
  await expect(page.locator("#displayNameCopy")).toContainText(
    "continue as a guest",
  );
...
test("shows provisional content before delayed websocket sync and upgrades to collaborative editing", async ({
  page,
}) => {
...
test("escapes raw html in markdown preview", async ({ page }) => {
  ...
  await expect(page.locator("#previewContent p").first()).toHaveText(
    "Line oneLine two",
  );
  await expect(
    page.locator("#previewContent p").first().locator("br"),
  ).toHaveCount(1);
  await expect(page.locator("#previewContent")).toContainText(
    "<script>window.__collabmdXss = true</script>",
  );
...
test("video toolbar helper converts a selected video url into markdown embed syntax", async ({
  page,
}) => {
  await openFile(page, "README.md");
  await replaceEditorContent(
  ...
  await expect(
    page.locator("#previewContent .video-embed-iframe"),
  ).toBeVisible();
  await expect(
    page.locator("#previewContent .video-embed-iframe"),
  ).toHaveAttribute(
...
test("image toolbar uploads a vault attachment and inserts inline markdown", async ({
  page,
}) => {
  await openFile(page, "README.md");
  ...
  await fileChooser.setFiles({
    buffer: Buffer.from(
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><rect width="24" height="24" fill="#0f172a"/></svg>',
    ),
...
test("image lightbox uses a fullscreen stage with click zoom, reset, and close", async ({
  page,
}) => {
  await openFile(page, "README.md");
  ...
  await fileChooser.setFiles({
    buffer: Buffer.from(
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48"><rect width="48" height="48" rx="8" fill="#0f172a"/><circle cx="24" cy="24" r="10" fill="#f8fafc"/></svg>',
    ),
    mimeType: "image/svg+xml",
  ...
  await page
    .locator(".image-lightbox-controls")
    .getByText("Reset", { exact: true })
    .click();
  await expect(zoomLabel).toHaveText("100%");
...
test("switching from a YouTube markdown preview to an image file clears the video overlay", async ({
  page,
}) => {
  await openFile(page, "README.md");
  ...
  await expect(
    page.locator('#previewContent [data-video-overlay-root="true"]'),
  ).toBeVisible();

  await page.locator("#fileTree").getByText("README.assets").click();
  await page
    .locator("#fileTree .file-tree-item", { hasText: "preview-switch" })
    .click();

  await expect(
    page.locator("#previewContent").locator(".image-file-preview-image"),
  ).toBeVisible();
  await expect(page.locator("#previewContent .video-embed-iframe")).toHaveCount(
    0,
  );
  await expect(
    page.locator('#previewContent [data-video-overlay-root="true"]'),
...
test("pasting an image uploads a vault attachment and inserts inline markdown", async ({
  page,
}) => {
  await openFile(page, "README.md");
...
test("opens a file by clicking the sidebar", async ({ page }) => {
  ...
  await page
    .locator("#fileTree .file-tree-dir", { hasText: "projects" })
    .first()
    .click();
  await page
    .locator("#fileTree .file-tree-item", { hasText: "collabmd" })
    .first()
    .click();
...
test("creates files from the sidebar with the custom dialog", async ({
  page,
}) => {
  await openHome(page);
  ...
  await expect(page.locator("#fileActionTitle")).toHaveText(
...
test("creates empty folders from the sidebar with the custom dialog", async ({
  page,
}) => {
  await openHome(page);
...
test("creates files inside a folder from the tree context menu", async ({
  page,
}) => {
  await openHome(page);

  const dailyFolder = page
    .locator("#fileTree .file-tree-dir", { hasText: "daily" })
    .first();
  await dailyFolder.click({ button: "right" });
  await expect(page.locator(".file-context-menu")).toBeVisible();
  await page
    .locator(".file-context-menu")
    .getByRole("button", { name: "New markdown file" })
    .click();

  await expect(page.locator("#fileActionDialog")).toBeVisible();
  await expect(page.locator("#fileActionNote")).toContainText(
    "Parent folder: daily",
  );
...
test("creates root files from empty tree space context menu", async ({
  page,
}) => {
  await openHome(page);
  ...
  await page
    .locator(".file-context-menu")
    .getByRole("button", { name: "New PlantUML diagram" })
    .click();
...
test("renames and deletes files from the sidebar with the custom dialog", async ({
  page,
}) => {
  await openHome(page);
  ...
  await waitForEditor(page);
  const scratchpadItem = page
    .locator("#fileTree .file-tree-item", { hasText: "scratchpad" })
    .first();
  ...
  const renamedItem = page
    .locator("#fileTree .file-tree-item", { hasText: "release-notes" })
    .first();
...
test("redundant hashchange events do not reopen the same markdown file into overlapping sessions", async ({
  page,
}) => {
  ...
  expect(
    (persistedContent.match(/CollabMD — Technical Design Document/g) || [])
      .length,
  ).toBe(1);
});
