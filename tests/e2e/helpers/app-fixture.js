...
export const ACTIVE_MAXIMIZED_EXCALIDRAW_SELECTOR =
  '[data-excalidraw-maximized-root="true"] .excalidraw-embed.is-maximized';
export const ACTIVE_MAXIMIZED_PLANTUML_SELECTOR =
  '[data-plantuml-maximized-root="true"] .plantuml-shell.is-maximized';
...
async function resetE2EAppState(
  page,
  { attempts = 5, stabilityWindowMs = 650 } = {},
) {
  let lastContent = "";
  const resetServerState = async () => {
    const response = await page.request.post(
      "http://127.0.0.1:4173/api/test/reset-state",
    );
    if (!response.ok()) {
      throw new Error(
        `reset-state failed: ${response.status()} ${await response.text()}`,
      );
    }
  };
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    const readReadme = async () => {
      const response = await page.request.get(
        "http://127.0.0.1:4173/api/file?path=README.md",
      );
    ...
    if (
      lastContent.includes("# My Vault") &&
      lastContent.includes("Welcome to the test vault")
    ) {
  ...
  throw new Error(
    `Failed to restore the E2E vault snapshot. README.md content was: ${lastContent}`,
  );
}
    ...
    .poll(
      async () =>
        page
          ...
          .evaluate((element) => element.dataset.editorMode || ""),
      { timeout: 15000 },
    )
    .toBe("collaborative");
}
...
export async function openFile(
  page,
  filePath,
  { userName = E2E_USER_NAME, waitFor = "editor" } = {},
) {
  ...
  if (waitFor === "loaded") {
    await expect
      .poll(async () => page.locator(".cm-editor").count(), { timeout: 15000 })
      .toBeGreaterThan(0);
    return;
  }
...
export async function openHome(page, { userName = E2E_USER_NAME } = {}) {
  ...
  await expect
    .poll(async () => page.locator("#fileTree .file-tree-item").count(), {
      timeout: 15000,
    })
    .toBeGreaterThan(0);
}
...
export async function setHydrateDelay(page, delayMs = 0) {
  const response = await page.request.post(
    "http://127.0.0.1:4173/api/test/hydrate-delay",
    {
      data: { delayMs },
    },
  );
  if (!response.ok()) {
    throw new Error(
      `hydrate-delay failed: ${response.status()} ${await response.text()}`,
    );
  }
}

export async function writeVaultFileAndResetCollab(page, { path, content }) {
  const resetResponseBeforeWrite = await page.request.post(
    "http://127.0.0.1:4173/api/test/reset-state",
  );
  if (!resetResponseBeforeWrite.ok()) {
    throw new Error(
      `reset-state failed before write: ${resetResponseBeforeWrite.status()} ${await resetResponseBeforeWrite.text()}`,
    );
  }

  await clearCollaborationSidecars(path);
  const writeResponse = await page.request.put(
    "http://127.0.0.1:4173/api/file",
    {
      data: { content, path },
    },
  );
  if (!writeResponse.ok()) {
    throw new Error(
      `write file failed: ${writeResponse.status()} ${await writeResponse.text()}`,
    );
  }

  const resetResponseAfterWrite = await page.request.post(
    "http://127.0.0.1:4173/api/test/reset-state",
  );
  if (!resetResponseAfterWrite.ok()) {
    throw new Error(
      `reset-state failed after write: ${resetResponseAfterWrite.status()} ${await resetResponseAfterWrite.text()}`,
    );
  }
}
...
export async function clearCollaborationSidecars(filePath) {
  ...
  await Promise.all(
    targets.map((target) => rm(target, { force: true }).catch(() => {})),
  );
}
...
export async function openSampleFull(
  page,
  { plantUmlLabel = "sample-full-plantuml" } = {},
) {
...
export async function duplicateVaultFile(page, sourcePath, targetPath) {
  await page.evaluate(
    async ({ sourcePath: source, targetPath: target }) => {
      ...
      if (typeof sourceData?.content !== "string") {
        throw new Error(
          sourceData?.error || `Missing file content for ${source}`,
        );
      }
...
export async function waitForExcalidrawTestHarness(page) {
  await expect
    .poll(async () =>
      page.evaluate(
        () => window.__COLLABMD_EXCALIDRAW_TEST__?.isReady?.() || false,
      ),
    )
    .toBe(true);
}

export async function waitForExcalidrawFrameHarness(
  page,
  selector = "#previewContent .excalidraw-embed iframe",
) {
...
export async function pasteClipboardImage(
  page,
  { buffer, fileName = "pasted-image.png", mimeType = "image/png" } = {},
) {
  ...
  const clipboardMode = await page.evaluate(
...
export async function setEditorSelection(
  ...
  await page.evaluate(
    ({ collapseAtStart, target }) => {
      ...
      const view =
        findView(editor) || findView(document.querySelector(".cm-content"));
      if (!view) {
...
export async function dragEditorSelection(page, targetText) {
  const coords = await page.evaluate((target) => {
    ...
    const view =
      findView(document.querySelector(".cm-editor")) ||
      findView(document.querySelector(".cm-content"));
    if (!view) {
      throw new Error("Missing CodeMirror editor view");
    }
...
export async function createComment(
  page,
  { body, collapseSelection = false, targetText, useInlineChip = false } = {},
) {
  if (targetText) {
...
export async function getHeavyPreviewCounts(page) {
  return page.evaluate(() => ({
    excalidrawIframes: document.querySelectorAll(
      "#previewContent .excalidraw-embed iframe",
    ).length,
    excalidrawPlaceholders: document.querySelectorAll(
      "#previewContent .excalidraw-embed-placeholder",
    ).length,
    plantumlShells: document.querySelectorAll("#previewContent .plantuml-shell")
      .length,
    plantumlSvgs: document.querySelectorAll(
      "#previewContent .plantuml-frame svg",
    ).length,
    mermaidShells: document.querySelectorAll("#previewContent .mermaid-shell")
      .length,
    mermaidSvgs: document.querySelectorAll("#previewContent .mermaid svg")
      .length,
    renderPhase:
      document.getElementById("previewContent")?.dataset.renderPhase || "",
    scrollHeight:
      document.getElementById("previewContainer")?.scrollHeight || 0,
  }));
}
...
export async function getMermaidZoomMetrics(page) {
  return page.evaluate(() => {
    const activeShell =
      document.querySelector("#previewContent .mermaid-shell.is-maximized") ||
...
export async function getPreviewHorizontalOverflowMetrics(
  page,
  {
  ...
  return page.evaluate(
    (selectors) => {
      ...
      const shell = document.querySelector(
        `#previewContent ${selectors.shellSelector}`,
      );
      const toolbar = document.querySelector(
        `#previewContent ${selectors.toolbarSelector}`,
      );
      const frame = document.querySelector(
        `#previewContent ${selectors.frameSelector}`,
      );
      const button = document.querySelector(
        `#previewContent ${selectors.buttonSelector}`,
      );
      ...
      return {
        ...
        shellRightOverflow: Math.max(
          0,
          Math.ceil(shellRect.right - containerRect.right),
        ),
        toolbarRightOverflow: Math.max(
          0,
          Math.ceil(toolbarRect.right - containerRect.right),
        ),
        maximizeButtonRightOverflow: Math.max(
          0,
          Math.ceil(buttonRect.right - containerRect.right),
        ),
      };
    },
    {
...
export async function getPreviewHeadingOffset(page, sourceLine) {
  return page.evaluate((line) => {
    ...
    const heading = document.querySelector(
      `#previewContent h2[data-source-line="${line}"]`,
    );
...
export function createScrollSyncRegressionDocument(itemCount = 80) {
  const lines = ["# Scroll Sync Regression", "", "## First section", ""];
