...
test.describe("mobile outline", () => {
  ...
  test("closes the outline after selecting a section on mobile", async ({
    page,
  }) => {
    ...
    await page
      .locator("#outlineNav .outline-item", { hasText: "Links" })
      .click();
...
test.describe("mobile linked mentions", () => {
  ...
  test("keeps linked mentions inline instead of showing the desktop dock", async ({
    page,
  }) => {
...
test.describe("mobile PlantUML preview", () => {
  ...
  test("keeps embedded and standalone PlantUML previews inside the mobile preview pane", async ({
    page,
  }) => {
    ...
    expect(
      embeddedMetrics.containerScrollWidth -
        embeddedMetrics.containerClientWidth,
    ...
    expect(embeddedMetrics.frameClientWidth).toBeLessThanOrEqual(
      embeddedMetrics.containerClientWidth,
    );
    ...
    expect(
      standaloneMetrics.containerScrollWidth -
        standaloneMetrics.containerClientWidth,
    ).toBeLessThanOrEqual(1);
    ...
    expect(standaloneMetrics.maximizeButtonRightOverflow).toBeLessThanOrEqual(
      1,
    );
    expect(standaloneMetrics.frameClientWidth).toBeGreaterThan(0);
    expect(standaloneMetrics.frameClientWidth).toBeLessThanOrEqual(
      standaloneMetrics.containerClientWidth,
    );
  });
});
...
test.describe("mobile video embeds", () => {
  ...
  test("keeps markdown video embeds inside the mobile preview pane", async ({
    page,
  }) => {
    await openFile(page, "README.md", { waitFor: "preview" });
    await expect(page.locator("#editorLayout")).toHaveAttribute(
    ...
    expect(metrics.frameClientWidth).toBeLessThanOrEqual(
...
test.describe("mobile sidebar", () => {
  ...
  test("closes the sidebar when tapping close button on mobile", async ({
    page,
  }) => {
    await openHome(page);
  ...
  test("closes the sidebar after selecting a file on mobile", async ({
    page,
  }) => {
    await openHome(page);
    ...
    await page
      .locator("#fileTree .file-tree-item", { hasText: "README" })
      .first()
      .click();
