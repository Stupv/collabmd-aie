...
test.describe("password auth", () => {
  ...
  test("requires a password before opening the editor and preserves the session across reloads", async ({
    page,
  }) => {
    ...
    await expect(page.locator(".auth-gate-error")).toContainText(
      "Incorrect password",
    );
  ...
  test("requires a new login in a fresh browser context", async ({
    browser,
  }) => {
  ...
  test("accepts a shared password from the URL fragment and removes it after login", async ({
    page,
  }) => {
    await page.goto(
      `${app.baseUrl}/#auth_password=${AUTH_PASSWORD}&file=test.md`,
    );
...
test.describe("oidc auth", () => {
  test("shows the Google login CTA, preserves the hash route, and syncs the Google identity", async ({
    page,
  }) => {
    ...
    await expect(page.locator(".auth-gate-card")).toBeVisible();
    await expect(page.locator(".auth-gate-secondary-button")).toHaveCount(0);
    await expect(page.locator(".auth-gate-button")).toHaveText(
      "Continue with Google",
    );
    await expect(page.locator(".auth-gate-button__icon img")).toHaveAttribute(
      "src",
    ...
    await expect
      .poll(async () =>
        page
          .locator(".auth-gate-button__icon img")
          .evaluate((node) =>
