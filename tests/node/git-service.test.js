...
async function createRemotePullFixture(t) {
  const remoteDir = await mkdtemp(
    join(tmpdir(), "collabmd-git-remote-fixture-"),
  );
...
test("GitService reports sections and diffs for staged, unstaged, and untracked files", async (t) => {
  ...
  const stagedFile = status.sections.find((section) => section.key === "staged")
    ?.files[0];
  const workingTreeFile = status.sections.find(
    (section) => section.key === "working-tree",
  )?.files[0];
  const untrackedFile = status.sections.find(
    (section) => section.key === "untracked",
  )?.files[0];
  ...
  assert.deepEqual(workingTreeDiff.files.map((file) => file.path).sort(), [
    "tracked.md",
    "untracked.md",
  ]);
  ...
  assert.deepEqual(fullDiff.files.map((file) => file.path).sort(), [
    "staged.md",
    "tracked.md",
    "untracked.md",
  ]);
  ...
  assert.deepEqual(metaDiff.files.map((file) => file.path).sort(), [
    "staged.md",
    "tracked.md",
    "untracked.md",
  ]);
  assert.equal(
    metaDiff.files.every((file) => !("hunks" in file)),
    true,
  );
});
...
test("GitService normalizes git paths that include spaces", async (t) => {
  ...
  const gitService = new GitService({ vaultDir: repoDir });
  const status = await gitService.getStatus({ force: true });
  const changedFile = status.sections.find(
    (section) => section.key === "working-tree",
  )?.files[0];
...
test("GitService guards large file patches until explicitly requested", async (t) => {
  ...
  const guardedDiff = await gitService.getDiff({
    path: "large.md",
    scope: "working-tree",
  });
  ...
  assert.equal(
    guardedCounter.calls.some(
      (args) => args[2] === "diff" && !args.includes("--numstat"),
    ),
    false,
  );
...
test("GitService avoids redundant subprocesses for status and meta diff requests", async (t) => {
  ...
  assert.equal(
    statusCounter.calls.some((args) => args.includes("rev-parse")),
    false,
  );
  ...
  assert.equal(
    diffCounter.calls.some((args) => args.includes("rev-parse")),
    false,
  );
});
...
test("GitService coalesces concurrent status and diff requests", async (t) => {
  ...
  assert.equal(
    firstStatus.summary.changedFiles,
    secondStatus.summary.changedFiles,
  );
  assert.equal(statusCounter.calls.length, 2);
...
test("GitService stages, unstages, and commits all staged changes", async (t) => {
  const repoDir = await mkdtemp(
    join(tmpdir(), "collabmd-git-service-actions-"),
  );
  t.after(async () => {
    await rm(repoDir, { force: true, recursive: true });
  });
  ...
  assert.equal(
    status.sections.find((section) => section.key === "staged")?.files.length,
    2,
  );
  ...
  assert.equal(
    status.sections
      .find((section) => section.key === "staged")
  ...
  assert.equal(
    status.sections
      .find((section) => section.key === "working-tree")
      ?.files.map((file) => file.path)
      .join(","),
    "a.md",
  );
  ...
  const headMessage = await execFile("git", ["log", "-1", "--pretty=%s"], {
    cwd: repoDir,
  });
  ...
  status = await gitService.getStatus({ force: true });
  assert.equal(
    (status.sections.find((section) => section.key === "staged")?.files ?? [])
      .length,
    0,
  );
  assert.equal(
    (
      status.sections.find((section) => section.key === "working-tree")
        ?.files ?? []
    ).length,
    0,
  );
...
test("GitService can commit staged changes when identity is provided through command env", async (t) => {
  const repoDir = await mkdtemp(
    join(tmpdir(), "collabmd-git-service-identity-"),
  );
  t.after(async () => {
    await rm(repoDir, { force: true, recursive: true });
  });
  ...
  await runGit(repoDir, ["commit", "-m", "Initial commit"]);
  await runGit(repoDir, ["config", "--unset-all", "user.name"]).catch(
    () => undefined,
  );
  await runGit(repoDir, ["config", "--unset-all", "user.email"]).catch(
    () => undefined,
  );
...
test("GitService can override commit identity per request author", async (t) => {
  ...
  const headAuthor = await execFile(
    "git",
    ["log", "-1", "--pretty=%an <%ae>"],
    { cwd: repoDir },
  );
  assert.equal(
    String(headAuthor.stdout).trim(),
    "Google User <google@example.com>",
  );
});
...
test("GitService pushes and pulls against an upstream branch", async (t) => {
  ...
  const remoteLog = await execFile("git", [
    "--git-dir",
    remoteDir,
    "log",
    "-1",
  ...
  const localContent = await execFile("git", ["show", "HEAD:test.md"], {
    cwd: localDir,
  });
...
test("GitService pull reports workspace changes from the fetched ref update", async (t) => {
  const remoteDir = await mkdtemp(
    join(tmpdir(), "collabmd-git-remote-change-"),
  );
  ...
  assert.deepEqual(pullResult.workspaceChange.changedPaths.sort(), [
    "new.md",
    "tracked.md",
  ]);
...
test("GitService pull uses autostash for non-overlapping dirty files without creating a backup", async (t) => {
  ...
  assert.equal(
    await readFile(join(localDir, "tracked.md"), "utf8"),
    "# Seed\nlocal dirty change\n",
  );
  assert.equal(
    await readFile(join(localDir, "remote.md"), "utf8"),
    "# Remote\nnew file\n",
  );
  assert.equal(
    (
      await execFile("git", ["status", "--porcelain=v1"], { cwd: localDir })
    ).stdout.trim(),
    "M tracked.md",
  );
});
...
test("GitService pull creates a backup bundle when tracked local edits overlap upstream changes", async (t) => {
  ...
  assert.equal(
    await readFile(
      join(
        localDir,
        pullResult.pullBackup.summaryPath.replace(
          "/summary.md",
          "/files/tracked.md",
        ),
      ),
...
test("GitService pull backup preserves staged and unstaged patch artifacts for partially staged overlaps", async (t) => {
  ...
  const backupRoot = join(
    localDir,
    pullResult.pullBackup.summaryPath.replace("/summary.md", ""),
  );
  const stagedPatch = await readFile(
    join(backupRoot, "patches/tracked.md.staged.patch"),
    "utf8",
  );
  const worktreePatch = await readFile(
    join(backupRoot, "patches/tracked.md.worktree.patch"),
    "utf8",
  );
  const summary = await readFile(
    join(localDir, pullResult.pullBackup.summaryPath),
    "utf8",
  );
...
test("GitService pull creates a backup bundle when an untracked local file overlaps a new upstream file", async (t) => {
  ...
  assert.equal(pullResult.pullBackup?.fileCount, 1);
  assert.equal(
    await readFile(join(localDir, "clash.md"), "utf8"),
    "# Remote\nincoming\n",
  );
  assert.equal(
    await readFile(
...
test("GitService pull rejects diverged history before mutating the worktree", async (t) => {
  ...
  await assert.rejects(
    ...
    (error) =>
      ...
      error?.requestCode === "pull_diverged_ff_only",
  );

  assert.equal(
    String(
      (await execFile("git", ["log", "-1", "--pretty=%s"], { cwd: localDir }))
        .stdout,
    ).trim(),
    "Local commit",
  );
...
test("GitService resets a file to the current branch content", async (t) => {
  ...
  const restored = await execFile("git", ["show", "HEAD:tracked.md"], {
    cwd: repoDir,
  });
  assert.equal(
    (
      await execFile("git", ["status", "--porcelain=v1"], { cwd: repoDir })
    ).stdout.trim(),
    "",
  );
...
test("GitService reset deletes files that do not exist on the current branch HEAD", async (t) => {
  const repoDir = await mkdtemp(
    join(tmpdir(), "collabmd-git-local-reset-delete-"),
  );
  t.after(async () => {
    await rm(repoDir, { force: true, recursive: true });
  });
...
test("GitService passes configured command env to subprocesses", async () => {
  ...
  assert.equal(
    observedEnvs.every(
      (env) => env?.COLLABMD_TEST_GIT_ENV === "expected-value",
    ),
    true,
  );
});
