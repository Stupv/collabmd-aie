...
test("normalizeCollaboratorViewport accepts finite viewport values", () => {
  ...
  assert.equal(
    normalizeCollaboratorViewport({ scrollX: 10, scrollY: 2, zoom: 0 }),
    undefined,
  );
  assert.equal(
    normalizeCollaboratorViewport({ scrollX: "x", scrollY: 2, zoom: 1 }),
    undefined,
  );
});
...
test("buildCollaboratorsMap preserves viewport awareness and peer lookup", () => {
  ...
  assert.deepEqual(collaborator.viewport, {
    scrollX: 40,
    scrollY: 90,
    zoom: 1.4,
  });
  assert.equal(
    findCollaboratorByPeerId(collaborators, "peer-remote"),
    collaborator,
  );
