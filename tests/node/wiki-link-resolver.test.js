...
test("resolveWikiTargetPath matches exact paths and bare note names", () => {
  const files = ["README.md", "notes/daily.md", "projects/collabmd.md"];
...
test("resolveWikiTargetWithIndex resolves without scanning file arrays", () => {
  ...
  assert.equal(
    resolveWikiTargetWithIndex("notes/daily", index),
    "notes/daily.md",
  );
  assert.equal(
    resolveWikiTargetWithIndex("collabmd", index),
    "projects/collabmd.md",
  );
});
