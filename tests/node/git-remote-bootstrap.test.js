...
async function createBareRemoteFixture(
  t,
  { initialContent = "# Test\n\nHello from remote.\n" } = {},
) {
...
function createBootstrapConfig(
  ...
  return loadConfig({
    git: {
      ...
      remote: {
        repoUrl: remoteDir,
        sshPrivateKeyBase64: Buffer.from("dummy-private-key", "utf8").toString(
          "base64",
        ),
      },
    },
...
test("prepareConfigForStartup clones the configured repo into an empty vault directory", async (t) => {
  ...
  const excludeContent = await readFile(
    join(vaultDir, ".git", "info", "exclude"),
    "utf8",
  );
  const repoUserName = (
    await execFile("git", ["config", "user.name"], { cwd: vaultDir })
  ).stdout.trim();
  const repoUserEmail = (
    await execFile("git", ["config", "user.email"], { cwd: vaultDir })
  ).stdout.trim();
...
test("prepareConfigForStartup fails when an existing checkout points to a different origin", async (t) => {
  ...
  const tempRoot = await mkdtemp(
    join(tmpdir(), "collabmd-bootstrap-mismatch-"),
  );
  const vaultDir = join(tempRoot, "vault");
  const config = createBootstrapConfig(vaultDir, remoteDir);
...
test("prepareConfigForStartup cleans up temporary key files created from base64 input", async (t) => {
  ...
  await assert.rejects(() => readFile(tempKeyPath, "utf8"), /ENOENT/);
});
...
test("server can start against a freshly bootstrapped vault and serve cloned files", async (t) => {
  ...
  const response = await httpRequest(
    `http://127.0.0.1:${port}/api/file?path=test.md`,
  );
