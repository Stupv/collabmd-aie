...
test("VaultFileStore can preserve the current collaboration snapshot during room-owned file persists", async (t) => {
  ...
  const snapshotWrite = await store.writeCollaborationSnapshot(
    "README.md",
    snapshot,
  );
  assert.equal(snapshotWrite.ok, true);

  const writeResult = await store.writeMarkdownFile(
    "README.md",
    "# Updated via room\n",
    {
  ...
  const invalidatingWrite = await store.writeMarkdownFile(
    "README.md",
    "# Updated via API\n",
  );
...
test("VaultFileStore persists content, comments, and snapshot as one staged collaboration update", async (t) => {
  ...
  assert.equal(
    await store.readMarkdownFile("README.md"),
    "# Updated atomically\n",
  );
  assert.deepEqual(await store.readCommentThreads("README.md"), threads);
  assert.deepEqual(
    Array.from((await store.readCollaborationSnapshot("README.md")) ?? []),
    Array.from(snapshot),
  );
});
...
test("VaultFileStore leaves live content untouched when staged collaboration snapshot preparation fails", async (t) => {
  ...
  assert.equal(
    await pathExists(join(vaultDir, ".collabmd/comments/README.md.json")),
    false,
  );
  assert.equal(
    await pathExists(join(vaultDir, ".collabmd/yjs/README.md.bin")),
    false,
  );
});
...
test("VaultFileStore persists hidden comment sidecars alongside vault files", async (t) => {
  ...
  const tree = await store.tree();
  assert.equal(
    tree.some((node) => node.name === ".collabmd"),
    false,
  );
...
test("VaultFileStore reads and writes PlantUML files", async (t) => {
  ...
  const createResult = await store.createFile(
    "diagram.puml",
    "@startuml\nAlice -> Bob: Hi\n@enduml\n",
  );
  ...
  const writeResult = await store.writePlantUmlFile(
    "diagram.puml",
    "@startuml\nBob -> Alice: Ack\n@enduml\n",
  );
  assert.equal(writeResult.ok, true);
...
test("VaultFileStore reads and writes Mermaid files", async (t) => {
  ...
  const writeResult = await store.writeMermaidFile(
    "diagram.mmd",
    "flowchart TD\n  B --> C\n",
  );
  assert.equal(writeResult.ok, true);
...
test("VaultFileStore writes image attachments next to their source markdown document", async (t) => {
  ...
  assert.equal(
    result.path,
    "README.assets/product-screenshot-20260316-153012.png",
  );
  assert.equal(
    result.markdownSnippet,
    "![Product Screenshot](README.assets/product-screenshot-20260316-153012.png)",
  );

  const attachment = await store.readImageAttachmentFile(result.path);
  assert.deepEqual(
    Array.from(attachment?.content ?? []),
    [0x89, 0x50, 0x4e, 0x47],
  );
  ...
  assert.deepEqual(
    assetsDirectory.children.map((node) => ({
      name: node.name,
      type: node.type,
    })),
