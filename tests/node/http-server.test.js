...
async function startPlantUmlStub() {
  ...
  const server = createServer((req, res) => {
    ...
    res.end(
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 40"><text x="8" y="24">stub</text></svg>',
    );
  });
...
test("HTTP server serves health, runtime config, and static assets", async (t) => {
  ...
  const runtimeConfigResponse = await httpRequest(
    `${app.baseUrl}/app-config.js`,
  );
  ...
  const assetHeadResponse = await httpRequest(
    `${app.baseUrl}/assets/css/style.css`,
    { method: "HEAD" },
  );
  assert.equal(assetHeadResponse.statusCode, 200);
  assert.equal(
    assetHeadResponse.headers["cache-control"],
    "public, max-age=0, must-revalidate",
  );

  const compressedAssetResponse = await httpRequest(
    `${app.baseUrl}/assets/css/style.css`,
    {
      headers: {
  ...
  assert.match(
    gunzipSync(compressedAssetResponse.bodyBuffer).toString("utf8"),
    /--color-bg/,
  );
});
...
test("HTTP server compresses large JSON API responses without changing payloads", async (t) => {
  ...
  const fileResponse = await httpRequest(
    `${app.baseUrl}/api/file?path=large.md`,
    {
      headers: {
        "Accept-Encoding": "gzip",
      },
  ...
  const payload = JSON.parse(
    gunzipSync(fileResponse.bodyBuffer).toString("utf8"),
  );
...
test("HTTP server serves prefixed routes when BASE_PATH is configured", async (t) => {
  ...
  const redirectResponse = await httpRequest(`${app.baseUrl}/collabmd`, {
    method: "HEAD",
  });
  ...
  const runtimeConfigResponse = await httpRequest(
    `${app.appBaseUrl}/app-config.js`,
  );
  assert.equal(runtimeConfigResponse.statusCode, 200);
  assert.match(runtimeConfigResponse.body, /"basePath":"\/collabmd"/);
  assert.match(
    runtimeConfigResponse.body,
    /"sessionEndpoint":"\/collabmd\/api\/auth\/session"/,
  );

  const assetResponse = await httpRequest(
    `${app.appBaseUrl}/assets/css/style.css`,
  );
  assert.equal(assetResponse.statusCode, 200);

  const unauthenticatedApiResponse = await httpRequest(
    `${app.appBaseUrl}/api/files`,
  );
  assert.equal(unauthenticatedApiResponse.statusCode, 401);

  const loginResponse = await httpRequest(
    `${app.appBaseUrl}/api/auth/session`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  const authenticatedApiResponse = await httpRequest(
    `${app.appBaseUrl}/api/files`,
    {
      headers: {
        Cookie: cookieHeader,
...
test("HTTP server exposes git status and diff endpoints for git-backed vaults", async (t) => {
  ...
  await execFile("git", ["config", "user.name", "CollabMD Tests"], {
    cwd: app.vaultDir,
    env: gitEnv,
  });
  ...
  const diffResponse = await httpRequest(
    `${app.baseUrl}/api/git/diff?scope=all`,
  );
  ...
  const metaDiffResponse = await httpRequest(
    `${app.baseUrl}/api/git/diff?scope=all&metaOnly=true`,
  );
  ...
  const cleanStatusResponse = await httpRequest(
    `${app.baseUrl}/api/git/status?force=true`,
  );
...
test("HTTP server exposes git push and pull endpoints for repos with an upstream", async (t) => {
  ...
  await execFile("git", ["push", "-u", "origin", "master"], {
    cwd: app.vaultDir,
    env: gitEnv,
  });
  ...
  await execFile("git", ["add", "test.md"], { cwd: app.vaultDir, env: gitEnv });
  await execFile("git", ["commit", "-m", "Local push commit"], {
    cwd: app.vaultDir,
    env: gitEnv,
  });
  ...
  await writeFile(
    join(peerDir, "test.md"),
    "# Test\n\nLocal push change.\nPeer pull change.\n",
    "utf8",
  );
  await execFile("git", ["add", "test.md"], { cwd: peerDir, env: gitEnv });
  await execFile("git", ["commit", "-m", "Peer pull commit"], {
    cwd: peerDir,
    env: gitEnv,
  });
  ...
  const fileResponse = await httpRequest(
    `${app.baseUrl}/api/file?path=test.md`,
  );
  assert.equal(fileResponse.statusCode, 200);
...
test("HTTP server returns pull backup metadata and lists saved pull backups", async (t) => {
  ...
  const remoteDir = await mkdtemp(
    join(tmpdir(), "collabmd-http-git-remote-backup-"),
  );
  const peerDir = await mkdtemp(
    join(tmpdir(), "collabmd-http-git-peer-backup-"),
  );
  t.after(async () => {
    await rm(remoteDir, { force: true, recursive: true });
    await rm(peerDir, { force: true, recursive: true });
  ...
  await execFile("git", ["add", "test.md"], { cwd: peerDir, env: gitEnv });
  await execFile("git", ["commit", "-m", "Remote overlap"], {
    cwd: peerDir,
    env: gitEnv,
  });
  await execFile("git", ["push"], { cwd: peerDir, env: gitEnv });
  ...
  const backupsResponse = await httpRequest(
    `${app.baseUrl}/api/git/pull-backups`,
  );
  assert.equal(backupsResponse.statusCode, 200);
  assert.match(
    backupsResponse.body,
    /"summaryPath":"\.collabmd\/pull-backups\/.*\/summary\.md"/,
  );
  ...
  const summaryResponse = await httpRequest(
    `${app.baseUrl}/api/file?path=${encodeURIComponent(summaryPath)}`,
  );
...
test("HTTP server returns a typed error code when pull cannot fast-forward", async (t) => {
  ...
  const remoteDir = await mkdtemp(
    join(tmpdir(), "collabmd-http-git-remote-diverged-"),
  );
  const peerDir = await mkdtemp(
    join(tmpdir(), "collabmd-http-git-peer-diverged-"),
  );
  t.after(async () => {
    await rm(remoteDir, { force: true, recursive: true });
    await rm(peerDir, { force: true, recursive: true });
  ...
  await execFile("git", ["add", "test.md"], { cwd: app.vaultDir, env: gitEnv });
  await execFile("git", ["commit", "-m", "Local commit"], {
    cwd: app.vaultDir,
    env: gitEnv,
  });
  ...
  await execFile("git", ["add", "test.md"], { cwd: peerDir, env: gitEnv });
  await execFile("git", ["commit", "-m", "Peer commit"], {
    cwd: peerDir,
    env: gitEnv,
  });
  await execFile("git", ["push"], { cwd: peerDir, env: gitEnv });
...
test("HTTP git reset invalidates stale collaboration snapshots so reopening hydrates from disk", async (t) => {
  ...
  await app.server.vaultFileStore.writeCollaborationSnapshot(
    "test.md",
    Y.encodeStateAsUpdate(staleDoc),
  );
  ...
  await waitForCondition(async () => {
    const snapshot =
      await app.server.vaultFileStore.readCollaborationSnapshot("test.md");
  ...
  assert.equal(
    reopenedDoc.getText("codemirror").toString(),
...
test("HTTP server supports password auth without blocking static assets", async (t) => {
  ...
  const unauthenticatedApiResponse = await httpRequest(
    `${app.baseUrl}/api/files`,
  );
  assert.equal(unauthenticatedApiResponse.statusCode, 401);
  assert.match(unauthenticatedApiResponse.body, /Authentication required/);

  const badLoginResponse = await httpRequest(
    `${app.baseUrl}/api/auth/session`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
test("HTTP server rejects invalid HTML payloads from esm.sh module requests", async (t) => {
  ...
  globalThis.fetch = async () =>
    new Response("<!DOCTYPE html><html><body>bad edge cache</body></html>", {
      headers: {
        "Content-Type": "application/javascript; charset=utf-8",
      },
      status: 200,
  ...
  const response = await httpRequest(
    `${app.baseUrl}/_esm/d3-fetch@3?target=es2022`,
  );
...
test("HTTP server rejects cross-origin write requests", async (t) => {
  ...
  const fileResponse = await httpRequest(
    `${app.baseUrl}/api/file?path=${encodeURIComponent("blocked.md")}`,
  );
...
test("HTTP server rejects unsupported /api/file mutations outside the vault file set", async (t) => {
  ...
  const deleteResponse = await httpRequest(
    `${app.baseUrl}/api/file?path=${encodeURIComponent("secret.txt")}`,
    {
      method: "DELETE",
    },
  );
  ...
  assert.match(
    renameResponse.body,
    /Old path must be a vault file \(\.md, .*\.png, .*\.svg\)/i,
  );
});
...
test("HTTP server uploads and serves vault-owned image attachments", async (t) => {
  ...
  assert.match(
    uploadResponse.body,
    /"markdown":"!\[Product Screenshot\]\(test\.assets\/product-screenshot-/,
  );
  assert.match(
    uploadResponse.body,
    /"path":"test\.assets\/product-screenshot-[^"]+\.png"/,
  );

  const uploadedPath = JSON.parse(uploadResponse.body).path;
  const attachmentResponse = await httpRequest(
    `${app.baseUrl}/api/attachment?path=${encodeURIComponent(uploadedPath)}`,
  );
  ...
  assert.deepEqual(
    Array.from(attachmentResponse.bodyBuffer),
    [0x89, 0x50, 0x4e, 0x47],
  );
...
test("HTTP server rejects invalid attachment uploads", async (t) => {
  ...
  const missingSourceResponse = await httpRequest(
    `${app.baseUrl}/api/attachments`,
    {
      body: Buffer.from([0x89, 0x50, 0x4e, 0x47]),
      headers: {
        "Content-Type": "image/png",
  ...
  const invalidTypeResponse = await httpRequest(
    `${app.baseUrl}/api/attachments`,
    {
...
test("HTTP server decodes encoded attachment metadata headers and hardens SVG responses", async (t) => {
  ...
  assert.match(
    uploadBody.markdown,
    /!\[diagram résumé\]\(caf%C3%A9\.assets\/diagram-r-sum-/,
  );
  assert.match(
    uploadBody.path,
    /^catatan\/café\.assets\/diagram-r-sum-[^/]+\.svg$/,
  );

  const attachmentResponse = await httpRequest(
    `${app.baseUrl}/api/attachment?path=${encodeURIComponent(uploadBody.path)}`,
  );
  assert.equal(attachmentResponse.statusCode, 200);
  ...
  assert.match(
    ...
    new RegExp(
      `filename\\*=UTF-8''${encodeURIComponent(basename(uploadBody.path))}`,
    ),
  );
});
...
test("HTTP server reads and writes .mmd files through /api/file", async (t) => {
  ...
  const readResponse = await httpRequest(
    `${app.baseUrl}/api/file?path=${encodeURIComponent("architecture.mmd")}`,
  );
  ...
  const updatedReadResponse = await httpRequest(
    `${app.baseUrl}/api/file?path=${encodeURIComponent("architecture.mmd")}`,
  );
...
test("HTTP server reads and writes .plantuml files through /api/file", async (t) => {
  ...
  const readResponse = await httpRequest(
    `${app.baseUrl}/api/file?path=${encodeURIComponent("architecture.plantuml")}`,
  );
  assert.equal(readResponse.statusCode, 200);
