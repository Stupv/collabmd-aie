...
test("GitService stays a thin facade over internal git modules", async () => {
  const source = await readSource(
    "src/server/infrastructure/git/git-service.js",
  );
  const specifiers = extractSpecifiers(source);
  ...
  assert.equal(
    specifiers.some((specifier) => specifier === "yjs"),
    false,
  );
...
test("CollabMdAppShell remains the composition root", async () => {
  ...
  assert.equal(
    specifiers.some((specifier) => specifier === "ws"),
    false,
  );
  assert.equal(
    specifiers.includes("../application/workspace-coordinator.js"),
    true,
  );
  assert.equal(
    specifiers.includes("../presentation/file-explorer-controller.js"),
    true,
  );
