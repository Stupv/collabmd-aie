...
import {
  parseStatusOutput,
  parseUnifiedDiff,
} from "../../src/server/infrastructure/git/parsers.js";
...
test("normalizeRelativeGitPath trims and rejects traversal", () => {
  ...
  assert.equal(
    normalizeRelativeGitPath('"BFI - Biller/sample flow.puml"'),
    "BFI - Biller/sample flow.puml",
  );
  assert.throws(
...
test("parseStatusOutput groups staged, working tree, and untracked files", () => {
  ...
  assert.deepEqual(
    parsed.sections.staged.map((file) => ({
  ...
  assert.deepEqual(
    parsed.sections["working-tree"].map((file) => ({
      path: file.path,
      status: file.status,
    })),
  ...
  assert.deepEqual(
    parsed.sections.untracked.map((file) => ({
      path: file.path,
      status: file.status,
    })),
