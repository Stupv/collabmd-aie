...
test("migrates legacy codemirror scene content into structured Excalidraw room state", () => {
  const doc = new Y.Doc();
  const legacyScene = JSON.stringify(
    createScene([createElement("shape-legacy")]),
  );
...
test("merges concurrent structured updates for different elements into one valid scene", () => {
  ...
  docA.transact(() => {
    applySceneDiffToExcalidrawRoom(
      docA,
      createScene([createElement("shape-a", { index: "a1", x: 10 })]),
    );
  }, "client-a");
  docB.transact(() => {
    applySceneDiffToExcalidrawRoom(
      docB,
      createScene([createElement("shape-b", { index: "a2", x: 30 })]),
    );
  }, "client-b");
  ...
  assert.deepEqual(
    sceneA.elements.map((element) => element.id),
    ["shape-a", "shape-b"],
  );
  assert.deepEqual(
    sceneB.elements.map((element) => element.id),
    ["shape-a", "shape-b"],
  );
});
...
test("keeps the higher element version and versionNonce when concurrent edits target the same element", () => {
  ...
  const baseScene = createScene([
    createElement("shared-shape", { version: 1, versionNonce: 1, x: 0 }),
  ]);
