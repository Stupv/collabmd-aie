...
function createScene(elementId, { color = "#ffffff" } = {}) {
...
test("ExcalidrawRoomClient syncs local scene updates into the structured room state", async () => {
  ...
  assert.deepEqual(
    buildExcalidrawRoomScene(ydoc).elements.map((element) => element.id),
    ["shape-1"],
  );
...
test("ExcalidrawRoomClient commitSceneJson writes the latest scene without tracking room history", async () => {
  ...
  assert.equal(
    client.commitSceneJson(createScene("shape-1", { color: "#111111" }), {
      origin: "test-1",
    }),
    true,
  );
  assert.equal(
    client.commitSceneJson(createScene("shape-2", { color: "#222222" }), {
      origin: "test-2",
    }),
    true,
  );
...
test("ExcalidrawRoomClient avoids rewriting the room when the scene is unchanged", async () => {
  ...
  assert.equal(
    client.commitSceneJson(createScene("shape-1"), { origin: "test-1-repeat" }),
    false,
  );
  assert.deepEqual(
    Array.from(Y.encodeStateAsUpdate(ydoc)),
    Array.from(updateBaseline),
  );
});
...
test("ExcalidrawRoomClient updates awareness fields for local user, pointer, and selection state", async () => {
  ...
  client.scheduleLocalPointerAwareness({
    button: "down",
    pointer: { tool: "laser", x: 10, y: 20 },
  });
  client.scheduleLocalViewportAwareness({
    scrollX: 12,
    scrollY: 34,
    zoom: 1.5,
  });
  ...
  assert.deepEqual(provider.awareness.localState.selectedElementIds, {
    shapeA: true,
  });
  assert.deepEqual(provider.awareness.localState.pointer, {
    tool: "laser",
    x: 10,
    y: 20,
  });
  assert.equal(provider.awareness.localState.pointerButton, "down");
  assert.deepEqual(provider.awareness.localState.viewport, {
    scrollX: 12,
    scrollY: 34,
    zoom: 1.5,
  });
