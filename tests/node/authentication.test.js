...
function withAuthEnvCleared(fn) {
  ...
  const previousGitPrivateKeyFile =
    process.env.COLLABMD_GIT_SSH_PRIVATE_KEY_FILE;
  const previousGitPrivateKeyBase64 =
    process.env.COLLABMD_GIT_SSH_PRIVATE_KEY_B64;
  const previousGitKnownHostsFile =
...
test("loadConfig defaults auth to none", () =>
  withAuthEnvCleared(() => {
...
test("password auth generates a password when one is not provided", () =>
  withAuthEnvCleared(() => {
    const config = loadConfig({
      auth: {
...
test("loadConfig requires OIDC base URL and credentials", () =>
  withAuthEnvCleared(() => {
    assert.throws(() => {
      loadConfig({
        auth: {
...
test("oidc auth exposes google runtime config when configured", () =>
  withAuthEnvCleared(() => {
    const config = loadConfig({
      auth: {
        oidc: {
    ...
    assert.equal(
      authService.getClientConfig().loginEndpoint,
      "/api/auth/oidc/login",
    );
...
test("password auth returns transport-agnostic session results and API authorization decisions", () => {
  ...
  assert.deepEqual(authService.authorizeApiRequest(authorizedRequest), {
    ok: true,
  });
});
...
test("oidc auth completes login, returns user status, and authorizes API requests", async (t) => {
  ...
  const loginRequestUrl = new URL(
    "http://127.0.0.1:3000/api/auth/oidc/login?returnTo=%2F%23file%3Dtest.md",
  );
  ...
  assert.match(
    ...
    new RegExp(`^${issuer.issuer}/authorize\\?`),
  );
  ...
  const callbackResult = await authService.completeOidcLogin(
    ...
    new URL(
      `http://127.0.0.1:3000/api/auth/oidc/callback?code=test-code&state=${encodeURIComponent(flowPayload.state)}`,
    ),
  );
  ...
  assert.match(
    issuer.getLastTokenRequestBody(),
  ...
  assert.equal(
    authService.getAuthenticatedUser(authenticatedRequest).sub,
    "google-sub",
  );
  assert.deepEqual(authService.authorizeApiRequest(authenticatedRequest), {
    ok: true,
  });
});
...
test("oidc auth adds the Google hosted-domain hint when exactly one allowed domain is configured", async (t) => {
  ...
  assert.equal(
    new URL(loginResult.redirectTo).searchParams.get("hd"),
    "company.com",
  );
});
...
test("oidc auth rejects users outside the allowed email and domain lists", async (t) => {
  ...
  assert.match(
    ...
    /auth_error=This\+Google\+account\+is\+not\+in\+the\+allowed\+email\+or\+domain\+list\./,
  );
});
...
test("loadConfig keeps remote bootstrap disabled when git repo env is absent", () =>
  withAuthEnvCleared(() => {
    const config = loadConfig({
      vaultDir: process.cwd(),
    });
...
test("loadConfig rejects remote bootstrap without a private key source", () =>
  withAuthEnvCleared(() => {
...
test("loadConfig captures git bootstrap env and prefers file over base64 when both are set", () =>
  withAuthEnvCleared(() => {
    process.env.COLLABMD_GIT_REPO_URL = "git@github.com:example/private.git";
    process.env.COLLABMD_GIT_SSH_PRIVATE_KEY_FILE = "./secrets/id_ed25519";
    process.env.COLLABMD_GIT_SSH_PRIVATE_KEY_B64 = Buffer.from(
      "dummy private key",
      "utf8",
    ).toString("base64");
...
test("loadConfig falls back to standard git author env for identity", () =>
  withAuthEnvCleared(() => {
