...
test("compilePreviewDocument keeps Mermaid keys stable across unrelated markdown edits", () => {
  const baseMarkdown = ["```mermaid", "graph TD", "  A-->B", "```"].join("\n");
  ...
  const editedHtml = compilePreviewDocument({
...
test("compilePreviewDocument renders inline br tags without enabling arbitrary html", () => {
  ...
  assert.match(
    html,
    /Template for BUY\/SELL stocks txn <br> <code>partnerCorporateAccountNo<\/code>/,
  );
...
test("compilePreviewDocument renders YouTube markdown images as no-cookie embeds", () => {
  ...
  assert.match(
    html,
    /class="video-embed-placeholder video-embed-shell diagram-preview-shell"/,
  );
  assert.match(html, /data-video-embed-kind="youtube"/);
  assert.match(
    html,
    /data-video-embed-url="https:\/\/www\.youtube-nocookie\.com\/embed\/dQw4w9WgXcQ"/,
  );
...
test("compilePreviewDocument renders direct https mp4 markdown images as native video", () => {
  ...
  assert.match(
    html,
    /data-video-embed-url="https:\/\/cdn\.example\.com\/videos\/demo\.mp4"/,
  );
...
test("compilePreviewDocument renders direct https webm markdown images as native video", () => {
  const markdown =
    "![WebM demo](https://cdn.example.com/videos/demo.webm?download=0)";
...
test("compilePreviewDocument keeps unsupported image urls as images", () => {
  ...
  assert.match(
    html,
    /<img src="https:\/\/cdn\.example\.com\/screenshot\.png" alt="Screenshot">/,
  );
...
test("compilePreviewDocument does not embed Google Drive public links", () => {
  const markdown =
    "![Drive video](https://drive.google.com/file/d/abc123/view?usp=sharing)";

  const { html } = compilePreviewDocument({ markdownText: markdown });

  assert.match(
    html,
    /<img src="https:\/\/drive\.google\.com\/file\/d\/abc123\/view\?usp=sharing" alt="Drive video">/,
  );
  assert.doesNotMatch(html, /video-embed/);
});

test("large-document classification triggers on any configured threshold", () => {
  const largeByChars = analyzeMarkdownComplexity(
    "a".repeat(LARGE_DOCUMENT_CHAR_THRESHOLD),
  );
  assert.equal(isLargeDocumentStats(largeByChars), true);

  const largeByMermaid = analyzeMarkdownComplexity(
    "```mermaid\ngraph TD\nA-->B\n```\n".repeat(20),
  );
  assert.equal(isLargeDocumentStats(largeByMermaid), true);

  const largeByMermaidEmbed = analyzeMarkdownComplexity(
    "![[diagram.mmd]]\n".repeat(20),
  );
  assert.equal(isLargeDocumentStats(largeByMermaidEmbed), true);

  const largeByExcalidraw = analyzeMarkdownComplexity(
    "![[diagram.excalidraw]]\n".repeat(8),
  );
  assert.equal(isLargeDocumentStats(largeByExcalidraw), true);

  const largeByPlantUml = analyzeMarkdownComplexity(
    "```plantuml\n@startuml\nAlice -> Bob: Hi\n@enduml\n```\n".repeat(12),
  );
  assert.equal(isLargeDocumentStats(largeByPlantUml), true);

  const largeByPlantUmlEmbed = analyzeMarkdownComplexity(
    "![[diagram.puml]]\n".repeat(12),
  );
  assert.equal(isLargeDocumentStats(largeByPlantUmlEmbed), true);

  const largeByPlantUmlLongEmbed = analyzeMarkdownComplexity(
    "![[diagram.plantuml]]\n".repeat(12),
  );
