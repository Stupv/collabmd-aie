...
test("CollaborationRoom allows a single oversized initial sync frame from an empty buffer", async () => {
  ...
  assert.equal(
    getSyncSubmessageType(client.sent[0]),
...
test("CollaborationRoom reuses cached initial sync payload until the document changes", async () => {
  ...
  room.doc.transact(() => {
    room.doc
      .getText("codemirror")
      .insert(room.doc.getText("codemirror").length, "updated");
...
test("CollaborationRoom hydrates and persists markdown comment threads", async () => {
  ...
  assert.equal(
    commentWrites[0].threads[0].messages[0].reactions[0].emoji,
    "👍",
  );
...
test("CollaborationRoom hydrates and persists excalidraw rooms via excalidraw file APIs", async () => {
  ...
  const room = new CollaborationRoom({
    ...
    vaultFileStore: {
      ...
      async readMarkdownFile() {
        throw new Error(
          "readMarkdownFile should not be called for .excalidraw rooms",
        );
      },
      ...
      async writeMarkdownFile() {
        throw new Error(
          "writeMarkdownFile should not be called for .excalidraw rooms",
        );
      },
    },
  });
  ...
  assert.deepEqual(
    buildExcalidrawRoomScene(room.doc).elements.map((element) => element.id),
    ["shape-1"],
  );
...
test("CollaborationRoom keeps the latest excalidraw state available while final persist is still running", async () => {
  ...
  assert.deepEqual(
    buildExcalidrawRoomScene(reconnectingRoom.doc).elements.map(
      (element) => element.id,
    ),
    ["shape-live"],
  );
  ...
  const room = new CollaborationRoom({
    ...
    vaultFileStore: {
      async persistCollaborationState(path) {
        ...
        maxConcurrentPersists = Math.max(
...
test("CollaborationRoom hydrates and persists PlantUML rooms via PlantUML file APIs", async () => {
  ...
  const room = new CollaborationRoom({
    ...
    vaultFileStore: {
      ...
      async readMarkdownFile() {
        throw new Error(
          "readMarkdownFile should not be called for .puml rooms",
        );
      },
      ...
      async writeMarkdownFile() {
        throw new Error(
          "writeMarkdownFile should not be called for .puml rooms",
        );
      },
    },
  });
...
test("CollaborationRoom hydrates and persists Mermaid rooms via Mermaid file APIs", async () => {
  ...
  const room = new CollaborationRoom({
    ...
    backlinkIndex: {
      updateFile() {
        throw new Error(
          "backlink index should not be updated for Mermaid files",
        );
      },
    },
    vaultFileStore: {
      ...
      async writeMarkdownFile() {
        throw new Error(
          "writeMarkdownFile should not be called for .mmd rooms",
        );
      },
    },
  });
...
test("CollaborationRoom hydrates and persists .plantuml rooms via PlantUML file APIs", async () => {
  ...
  const room = new CollaborationRoom({
    ...
    backlinkIndex: {
      updateFile() {
        throw new Error(
          "backlink index should not be updated for PlantUML files",
        );
      },
    },
    vaultFileStore: {
      ...
      async readMarkdownFile() {
        throw new Error(
          "readMarkdownFile should not be called for .plantuml rooms",
        );
      },
      async writePlantUmlFile(path, content) {
        writes.push({ content, path });
      ...
      async writeMarkdownFile() {
        throw new Error(
          "writeMarkdownFile should not be called for .plantuml rooms",
        );
      },
    },
  });
