...
function renderOidcPrompt(card, config, { errorMessage = "" }) {
  ...
  signInButton.append(
    createGoogleIcon(),
    document.createTextNode(config.submitLabel || "Continue with Google"),
  );
...
function renderStatusCard(
  card,
  { title, body, secondaryActionLabel = "", onSecondaryAction = null },
) {
  card.replaceChildren();
...
function renderPasswordPrompt(card, config, { onSubmit }) {
  card.replaceChildren();
