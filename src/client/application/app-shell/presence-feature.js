export const presenceFeature = {
  ...
  renderPresence() {
    ...
    const isExcalidrawRoute = Boolean(
      this.currentFilePath && this.isExcalidrawFile?.(this.currentFilePath),
    );
    const isImageRoute = Boolean(
      this.currentFilePath && this.isImageFile?.(this.currentFilePath),
    );
    const shouldUseLobbyState =
      !hasEditorSession &&
      (!this.currentFilePath || isExcalidrawRoute || isImageRoute);
    ...
    if (effectiveConnectionState.status === "connecting") {
      badge.textContent = effectiveConnectionState.unreachable
        ? "Unreachable"
        : "Connecting...";
  ...
  renderAvatars() {
    ...
    const followedIndex = visibleUsers.findIndex(
      (u) => !u.isLocal && u.clientId === this.followedUserClientId,
    );
    ...
    visibleUsers.slice(0, 5).forEach((user) => {
      ...
      avatar.classList.toggle(
        "is-following",
        user.clientId === this.followedUserClientId,
      );
      ...
      const sameFile =
        user.currentFile && user.currentFile === this.currentFilePath;
      ...
      if (user.isLocal) {
        ...
        avatar.addEventListener("click", () =>
          this.toggleFollowUser(user.clientId),
        );
      }
  ...
  toggleFollowUser(clientId) {
    ...
    const user = this.globalUsers.find(
      (u) => u.clientId === clientId && !u.isLocal,
    );
    if (!user) return;
  ...
  syncFollowedUser() {
    if (!this.followedUserClientId) return;
    const user = this.globalUsers.find(
      (u) => u.clientId === this.followedUserClientId,
    );
  ...
  followUserCursor(user, { force = false } = {}) {
    ...
    const liveViewport =
      fileClientId != null ? this.session?.getUserViewport(fileClientId) : null;
    const liveCursor =
      fileClientId != null ? this.session?.getUserCursor(fileClientId) : null;
    ...
    if (
      !user ||
      (viewportTopLine == null && (cursorHead == null || cursorLine == null))
    ) {
      this.followedCursorSignature = "";
      return;
    }
    ...
    const didScroll =
      (fileClientId != null &&
        this.session?.scrollToUserViewport(fileClientId)) ||
      (fileClientId != null &&
        this.session?.scrollToUserCursor(fileClientId, "center")) ||
      (cursorHead != null &&
        this.session?.scrollToPosition(cursorHead, "center")) ||
      (cursorLine != null &&
        this.session?.scrollToLine(cursorLine, viewportRatio));
