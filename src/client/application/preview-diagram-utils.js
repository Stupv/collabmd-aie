...
export function getSvgSize(svg) {
  ...
  return {
    height:
      viewBox?.height || bbox?.height || attributeHeight || rect.height || 480,
...
export function createMermaidPlaceholderCardWithMessage(
  key,
  { label = "Mermaid diagram", message = "Loads when visible" } = {},
) {
...
export function createPlantUmlPlaceholderCard(
  key,
  message = "Renders server-side when visible",
) {
  const card = document.createElement("div");
...
export function shouldPreserveHydratedDiagram({
  nextSource = "",
  nextTarget = "",
  preservedSource = "",
  preservedTarget = "",
