...
function clampHeight(height) {
  return Math.max(
    MIN_HEIGHT,
    Math.min(MAX_HEIGHT, Math.ceil(height || DEFAULT_HEIGHT)),
  );
}
...
export class VideoEmbedController {
  ...
  reconcileEmbeds(previewElement = this.previewElement) {
    ...
    const descriptors = Array.from(
      previewElement.querySelectorAll(
        ".video-embed-placeholder[data-video-embed-key]",
      ),
    ).map((placeholder) => ({
    ...
    this.embedEntries.forEach((entry) => {
      ...
      entry.mimeType =
        entry.placeholder?.dataset.videoEmbedMimeType || entry.mimeType;
      entry.source =
        entry.placeholder?.dataset.videoEmbedSource || entry.source;
  ...
  _attachWrapper(entry) {
    const placeholder = entry.placeholder?.isConnected
      ...
      : this.previewElement?.querySelector(
          `.video-embed-placeholder[data-video-embed-key="${entry.key}"]`,
        );
  ...
  _syncEntryLayout(entry) {
    ...
    const height = clampHeight(
      width > 0
        ? width / (entry.aspectRatio || DEFAULT_ASPECT_RATIO)
