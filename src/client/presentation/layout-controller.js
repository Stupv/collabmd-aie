export class LayoutController {
  ...
  restorePreferredView() {
    const nextView = this.isMobileViewport()
      ? this.mobileShowsEditor
        ? "split"
        : "preview"
      : this.preferredView;

    this.applyView(nextView);
    this.updateMobileToggleButton(
      this.mobileShowsEditor ? "Preview" : "Editor",
    );
  }
  ...
  updateResizerValue() {
    ...
    const percentage = Math.round(
      (this.editorPane.offsetWidth / totalWidth) * 100,
    );
    this.resizer.setAttribute(
      "aria-valuenow",
      String(Math.max(20, Math.min(80, percentage))),
    );
  }
  ...
  applyEditorPaneWidth(width) {
    ...
    const nextWidth = Math.max(
      minPaneWidth,
      Math.min(totalWidth - minPaneWidth, width),
    );
