...
export class FileActionController {
  ...
  initialize() {
    ...
    this.newDrawingButton?.addEventListener("click", () =>
      this.handleNewDrawing(),
    );
    this.newMermaidButton?.addEventListener("click", () =>
      this.handleNewMermaid(),
    );
    this.newPlantumlButton?.addEventListener("click", () =>
      this.handleNewPlantUml(),
    );
    ...
    this.actionDialog?.addEventListener("close", () =>
  ...
  closeActionDialog() {
    ...
    if (
      ...
      typeof this.actionDialog.close === "function"
    ) {
      this.actionDialog.close();
      return;
    }
  ...
  setActionDialogBusy(isBusy) {
    ...
    if (this.actionInput) {
      this.actionInput.disabled =
        isBusy || !(this.pendingAction?.requiresInput ?? true);
    }
    if (this.actionCancelButton) {
  ...
  async handleActionSubmit() {
    ...
    const rawValue = requiresInput
      ? (this.actionInput?.value.trim() ?? "")
      : undefined;
  ...
  async createVaultFile(
    filePath,
    content,
    { openAfterCreate = false, errorMessage = "Failed to create file" } = {},
  ) {
    try {
  ...
  handleNewFile({ parentDir = "" } = {}) {
    ...
    this.openActionDialog({
      ...
      onSubmit: (value) => {
        ...
        const filePath = this.ensureExtension(
          composeVaultChildPath(context.normalizedParentDir, normalizedPath),
          ".md",
        );
    ...
    this.openActionDialog({
      ...
      onSubmit: (value) =>
        this.createDirectory(
          composeVaultChildPath(context.normalizedParentDir, value),
        ),
    });
  }
  ...
  handleNewDrawing({ parentDir = "" } = {}) {
    ...
    this.openActionDialog({
      ...
      note: context.note,
      placeholder: context.normalizedParentDir
        ? "architecture"
  ...
  handleNewMermaid({ parentDir = "" } = {}) {
    ...
    this.openActionDialog({
      ...
      onSubmit: (value) => {
        ...
        const starter = createMermaidStarter(
          composeVaultChildPath(context.normalizedParentDir, normalizedPath),
        );
  ...
  handleNewPlantUml({ parentDir = "" } = {}) {
    ...
    this.openActionDialog({
      ...
      onSubmit: (value) => {
        ...
        const starter = createPlantUmlStarter(
          composeVaultChildPath(context.normalizedParentDir, normalizedPath),
        );
        return this.createVaultFile(starter.path, starter.content, {
