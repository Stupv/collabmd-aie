...
function normalizePublicBaseUrl(rawValue) {
  ...
  if (
    ...
    throw new Error(
      "PUBLIC_BASE_URL must use https unless it points to localhost.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "PUBLIC_BASE_URL must not include a path, query string, or hash.",
    );
  }
...
function normalizeAuthStrategy(rawStrategy) {
  const normalized = String(rawStrategy ?? AUTH_STRATEGY_NONE)
    .trim()
    .toLowerCase();
...
function normalizeEmailAllowlist(value) {
  return normalizeCsvList(value).map((entry) => entry.toLowerCase());
}
...
function loadGitConfig(overrides = {}) {
  ...
  const enabled =
    overrides.enabled ?? process.env.COLLABMD_GIT_ENABLED !== "false";
  const repoUrl = normalizeOptionalString(
    remoteOverrides.repoUrl ?? process.env.COLLABMD_GIT_REPO_URL,
  );
...
function loadOidcConfig(overrides = {}, { basePath = "" } = {}) {
  const clientId = normalizeOptionalString(
    overrides.clientId ?? process.env.AUTH_OIDC_CLIENT_ID,
  );
  const clientSecret = normalizeOptionalString(
    overrides.clientSecret ?? process.env.AUTH_OIDC_CLIENT_SECRET,
  );
  ...
  const issuer =
    normalizeOptionalString(
      overrides.issuer ?? process.env.AUTH_OIDC_ISSUER_URL,
    ) || "https://accounts.google.com";
  const flowCookieName =
    normalizeOptionalString(
      overrides.flowCookieName ?? process.env.AUTH_OIDC_FLOW_COOKIE_NAME,
    ) || "collabmd_auth_flow";
  const allowedEmails = normalizeEmailAllowlist(
    overrides.allowedEmails ?? process.env.AUTH_OIDC_ALLOWED_EMAILS,
  );
  const allowedDomains = normalizeDomainAllowlist(
    overrides.allowedDomains ?? process.env.AUTH_OIDC_ALLOWED_DOMAINS,
  );
...
export function loadConfig(overrides = {}) {
  ...
  const authStrategy = normalizeAuthStrategy(
    authOverrides.strategy ?? process.env.AUTH_STRATEGY ?? AUTH_STRATEGY_NONE,
  );
  const passwordWasGenerated =
    authStrategy === AUTH_STRATEGY_PASSWORD &&
  ...
  const password =
    authStrategy === AUTH_STRATEGY_PASSWORD
      ...
      : "";
  const oidc =
    authStrategy === AUTH_STRATEGY_OIDC
  ...
  return {
    auth: {
      ...
      sessionCookieName:
        authOverrides.sessionCookieName ||
        process.env.AUTH_SESSION_COOKIE_NAME ||
        "collabmd_auth",
      sessionSecret: resolveSessionSecret(authOverrides),
      strategy: authStrategy,
    },
    basePath,
    host: process.env.HOST || getDefaultHost(nodeEnv),
    httpHeadersTimeoutMs: parsePositiveInt(
      process.env.HTTP_HEADERS_TIMEOUT_MS,
      60_000,
    ),
    httpKeepAliveTimeoutMs: parsePositiveInt(
      process.env.HTTP_KEEP_ALIVE_TIMEOUT_MS,
      5_000,
    ),
    httpRequestTimeoutMs: parsePositiveInt(
    ...
    testWsRoomHydrateDelayMs: parsePositiveInt(
      process.env.TEST_WS_ROOM_HYDRATE_DELAY_MS,
      0,
    ),
    wsHeartbeatIntervalMs: parsePositiveInt(
      process.env.WS_HEARTBEAT_INTERVAL_MS,
      30_000,
    ),
    wsRoomIdleGraceMs: parsePositiveInt(
      process.env.WS_ROOM_IDLE_GRACE_MS,
      60_000,
    ),
    ...
    wsMaxPayloadBytes: parsePositiveInt(
