...
async function applyWorkspaceMutationEffects({
  ...
  const workspaceChange =
    responsePayload?.workspaceChange ?? createEmptyWorkspaceChange();
...
export function createGitApiCommandHandler({
  ...
  return async function handleGitApiCommand(req, res, requestUrl) {
    if (requestUrl.pathname === "/api/git/stage" && req.method === "POST") {
      try {
        ...
        jsonResponse(
          ...
          res,
          200,
          await applyWorkspaceMutationEffects({
            action: "stage",
            req,
            responsePayload:
              await workspaceMutationCoordinator.runManagedWorkspaceMutation(
    ...
    if (requestUrl.pathname === "/api/git/unstage" && req.method === "POST") {
      try {
        ...
        handleGitError(
          ...
          res,
          error,
          "[api] Failed to unstage git file:",
    ...
    if (requestUrl.pathname === "/api/git/commit" && req.method === "POST") {
      try {
        ...
        handleGitError(
          ...
          res,
          error,
          "[api] Failed to commit staged changes:",
    ...
    if (
      requestUrl.pathname === "/api/git/reset-file" &&
      req.method === "POST"
    ) {
      try {
        const body = await parseRequiredBody(req, res, "path");
        ...
        handleGitError(
          ...
          res,
          error,
          "[api] Failed to reset git file:",
          "Failed to reset git file",
        );
      }
      return true;
    }
