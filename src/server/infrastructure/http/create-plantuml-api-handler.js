...
export function createPlantUmlApiHandler({ plantUmlRenderer = null }) {
  return async function handlePlantUmlApi(req, res, requestUrl) {
    if (
      !(requestUrl.pathname === "/api/plantuml/render" && req.method === "POST")
    ) {
      return false;
    }
    ...
    try {
      ...
      if (!plantUmlRenderer) {
        jsonResponse(req, res, 503, {
          error: "PlantUML renderer is not configured",
        });
        return true;
      }
      ...
      jsonResponse(req, res, statusCode, {
        error:
          error instanceof Error ? error.message : "Failed to render PlantUML",
      });
    }
