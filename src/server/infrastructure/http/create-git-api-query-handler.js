...
export function createGitApiQueryHandler({ gitService }) {
  return async function handleGitApiQuery(req, res, requestUrl) {
      ...
      try {
        jsonResponse(
          ...
          await gitService.getDiff({
            allowLargePatch: isTruthyParam(
              requestUrl.searchParams.get("allowLargePatch"),
            ),
    ...
    if (
      requestUrl.pathname === "/api/git/pull-backups" &&
      req.method === "GET"
    ) {
      try {
        ...
        handleGitError(
          ...
          res,
          error,
          "[api] Failed to read pull backups:",
          "Failed to read pull backups",
        );
      }
      return true;
    }
