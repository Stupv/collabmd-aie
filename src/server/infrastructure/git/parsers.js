...
function parseBranchLine(line) {
  ...
  const branchSegment =
    metadataIndex === -1 ? branchText : branchText.slice(0, metadataIndex);
  const metadataSegment =
    metadataIndex === -1 ? "" : branchText.slice(metadataIndex + 2, -1);
  ...
  return String(output ?? "")
    ...
    .reduce(
      (summary, line) => {
        ...
        const additions =
          rawAdditions === "-" ? 0 : Number.parseInt(rawAdditions, 10) || 0;
        const deletions =
          rawDeletions === "-" ? 0 : Number.parseInt(rawDeletions, 10) || 0;
...
export function parseNumstatEntries(output) {
  return String(output ?? "")
    ...
    .filter(Boolean)
    .map((line) => {
      const [rawAdditions = "0", rawDeletions = "0", ...rest] =
        line.split("\t");
      return {
        additions:
          rawAdditions === "-" ? 0 : Number.parseInt(rawAdditions, 10) || 0,
    ...
    .reduce(
      (summary, line) => {
        ...
        if (status.startsWith("R") || status.startsWith("C")) {
          const [oldPath = "", newPath = ""] = parts.map((value) =>
            decodeQuotedPath(value),
          );
...
export function parseUnifiedDiff(diffText) {
  ...
  return files.map((file) => finalizeFile(file)).filter(Boolean);
}
