...
function createWorkspaceEntry(relativePath, type) {
  return {
    ...
    path: relativePath,
    type:
      type === "directory" ? "directory" : getVaultTreeNodeType(relativePath),
  };
}

async function cleanupPaths(paths = []) {
  await Promise.allSettled(
    paths.filter(Boolean).map((pathValue) => rm(pathValue, { force: true })),
  );
}
...
function resolveAttachmentExtension({ mimeType, originalFileName }) {
  ...
  const extensionFromName = extname(
    String(originalFileName ?? ""),
  ).toLowerCase();
...
function createAttachmentMarkdownSnippet({
  altText,
  documentPath,
  storedPath,
}) {
  const relativePath = relative(dirname(documentPath), storedPath).replace(
    /\\/g,
    "/",
  );
  const encodedRelativePath = encodeMarkdownPath(
    relativePath || basename(storedPath),
  );
...
export class VaultFileStore {
  ...
  async writeContentFile(
    ...
    if (!resolved || resolved.adapter.kind !== expectedKind) {
      return {
        ok: false,
        error:
          resolved?.adapter?.invalidPathError ??
          getVaultContentAdapter(filePath)?.invalidPathError ??
          INVALID_VAULT_FILE_PATH_ERROR,
      };
    }
  ...
  async readImageAttachmentFile(filePath) {
    ...
    try {
      ...
      return {
        content,
        mimeType:
          IMAGE_EXTENSION_TO_MIME_TYPE[extname(filePath).toLowerCase()] ||
  ...
  async writeImageAttachmentForDocument(
    sourceDocumentPath,
    { content, mimeType, originalFileName = "", now = new Date() } = {},
  ) {
    ...
    do {
      ...
      absolutePath = this.resolveContentPath(storedPath, {
        requireVaultFile: false,
      });
      collisionIndex += 1;
  ...
  async persistCollaborationState(
    filePath,
    { commentThreads = [], content = "", snapshot = null } = {},
  ) {
    const resolved = this.resolveAdapter(filePath);
    if (!resolved) {
      return {
    ...
    const operations = [
      {
        kind:
          Array.isArray(commentThreads) && commentThreads.length > 0
            ? "write"
            : "delete",
    ...
    try {
      ...
      await cleanupPaths(
        committedOperations.map((operation) => operation.backupPath),
      );
      return { ok: true };
    } catch (error) {
  ...
  async renameFile(oldPath, newPath) {
    const { absoluteNew, absoluteOld, error } = resolveVaultRenamePaths(
      this.vaultDir,
      oldPath,
      newPath,
    );
  ...
  async createDirectory(dirPath) {
    const { absolute, error } = resolveVaultDirectoryPath(
      this.vaultDir,
      dirPath,
    );
    if (!absolute) {
    ...
    try {
      await this.runManagedWrite([dirPath], () =>
        mkdir(absolute, { recursive: true }),
      );
      return { ok: true };
    } catch (error) {
      return { ok: false, error: error.message };
  ...
  async reconcileSidecars({ deletedPaths = [], renamedPaths = [] } = {}) {
    await Promise.allSettled([
      ...Array.from(new Set((deletedPaths ?? []).filter(Boolean)), (filePath) =>
        this.sidecarStore.deleteAllForFile(filePath),
      ),
      ...Array.from(
        new Map(
          (renamedPaths ?? [])
        ...
        (entry) =>
          this.sidecarStore.renameAllForFile(entry.oldPath, entry.newPath),
      ),
    ]);
  }
  ...
  async reconcileCollaborationSnapshots({
    ...
    await Promise.allSettled(
      Array.from(affectedPaths, (filePath) =>
        this.deleteCollaborationSnapshot(filePath),
      ),
    );
  }
  ...
  resolveWikiLink(linkTarget) {
    const normalized = linkTarget.endsWith(".md")
      ? linkTarget
      : `${linkTarget}.md`;
    const absolute = this.resolveContentPath(normalized, {
      requireVaultFile: false,
    });
    if (!absolute) {
      return null;
