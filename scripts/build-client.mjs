...
const excalidrawEditorEntrySource = resolve(
  clientSourceDir,
  "excalidraw-editor.js",
);
const excalidrawMermaidStubSource = resolve(
  clientSourceDir,
  "excalidraw-mermaid-stub.js",
);
const previewWorkerSource = resolve(
  clientSourceDir,
  "application/preview-render-worker.js",
);
const previewWorkerOutput = resolve(
  clientOutputDir,
  "preview-render-worker.js",
);
const excalidrawLoaderOutput = resolve(clientOutputDir, "excalidraw-editor.js");
const excalidrawCssLoaderOutput = resolve(
  clientOutputDir,
  "excalidraw-editor.css",
);
  ...
  await Promise.all(
    jsFiles.map(async (filePath) => {
      ...
      for (const {
        pattern,
        replacement,
      } of deprecatedLifecycleListenerPatches) {
...
async function bundleStyles() {
  ...
  await Promise.all(
    styleAssetFiles.map(async (fileName) => {
      const source = await readFile(
        resolve(clientStyleSourceDir, fileName),
        "utf8",
      );
      ...
      await writeFile(
        resolve(clientStyleOutputDir, fileName),
        result.code,
        "utf8",
      );
    }),
...
try {
  ...
  await Promise.all(
    obsoleteOutputDirs.map((directory) =>
