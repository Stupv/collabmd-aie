...
const { resolveCliVaultDir, loadConfig } =
  await import("../src/server/config/env.js");
...
if (useLocalPlantUml) {
  const { getLocalPlantUmlServerUrl, startLocalPlantUmlComposeService } =
    await import("../scripts/local-plantuml-compose.mjs");

  try {
    const localPlantUmlUrl = getLocalPlantUmlServerUrl();
    console.log(
      `  PlantUML: starting local docker-compose service at ${localPlantUmlUrl}...`,
    );
    ...
    if (error.code === "ENOENT") {
      console.error(
        "Error: Docker is not available. Install Docker Desktop or Docker Engine first.",
      );
    } else {
      console.error(
        `Error: Failed to start local PlantUML service: ${error.message}`,
      );
    }
    process.exit(1);
  }
...
const { prepareConfigForStartup } =
  await import("../src/server/startup/git-remote-bootstrap.js");
